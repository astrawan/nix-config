...
let
  bgFileName =
    if themeConfig.General.background != null then
      lib.builtins.baseNameOf themeConfig.General.background
    else
      "noctalia.png";
  bgFinalConfigValue = "Assets/Wallpaper/${bgFileName}";
  finalThemeConfig =
    if themeConfig != null then
      themeConfig.override { General.background = bgFinalConfigValue; }
    else
      null;
in
